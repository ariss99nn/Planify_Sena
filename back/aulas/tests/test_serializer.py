...
class BloqueSerializerTest(TestCase):
    ...
    def test_create_valido(self):
        data = {'nombre': 'Bloque Test', 'piso': 4, 'capacidad_maxima': 300}
    ...
    def test_piso_cero_invalido(self):
        data = {'nombre': 'Bloque X', 'piso': 0, 'capacidad_maxima': 100}
        s = BloqueCreateSerializer(data=data)
        self.assertFalse(s.is_valid())
        self.assertIn('piso', s.errors)

    def test_capacidad_cero_invalida(self):
        data = {'nombre': 'Bloque Y', 'piso': 2, 'capacidad_maxima': 0}
        s = BloqueCreateSerializer(data=data)
        self.assertFalse(s.is_valid())
    ...
    def test_update_parcial(self):
        ...
        s = BloqueUpdateSerializer(bloque, data={'piso': 5}, partial=True)
        self.assertTrue(s.is_valid(), s.errors)
        s.save()
        bloque.refresh_from_db()
        self.assertEqual(bloque.piso, 5)
