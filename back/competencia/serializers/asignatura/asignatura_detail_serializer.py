...
class AsignaturaDetailSerializer(serializers.ModelSerializer):
    ...
    def get_docentes_asignados(self, obj):
        from competencia.serializers.docente_asignatura.docente_asignatura_list_serializer import (
            DocenteAsignaturaListSerializer,
        )
        return DocenteAsignaturaListSerializer(
            obj.docentes_asignados.filter(activo=True), many=True
        ).data