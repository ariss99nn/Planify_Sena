...
from competencia.serializers import CompetenciaListSerializer
from competencia.filters import CompetenciaFilter, CompetenciaPagination
...
class CompetenciaListView(CompetenciaBaseView):
    ...
    DOCENTE: solo competencias de sus asignaturas asignadas.
    ESTUDIANTE: solo competencias del módulo de su ficha.
    """
    permission_classes = [IsAuthenticated]

    def get_queryset(self, request):
        user = request.user
        qs = Competencia.objects.select_related('asignatura')
        ...
        if user.rol == User.Rol.DOCENTE:
            ...
            return qs.filter(asignatura_id__in=asignatura_ids)
        ...
        if user.rol == User.Rol.ESTUDIANTE:
            ...
            return qs.filter(asignatura__modulo_id__in=modulo_ids)
