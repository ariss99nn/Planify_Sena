...
class PlanificacionBaseView(APIView):
    ...
    def get_plan_or_404(self, pk):
        obj = PlanTrimestral.objects.select_related(
            'ficha__version__programa',
            'aprobado_por',
        ).prefetch_related(
            'items__competencia',
            'items__docente__user',
        ).filter(pk=pk).first()
        if obj is None:
    ...
    def get_item_or_404(self, pk):
        obj = ItemPlan.objects.select_related(
            'plan__ficha__version__programa',
            'competencia__asignatura',
            'docente__user',
        ).prefetch_related(
            'bloques_ejecutados__bloque',
        ).filter(pk=pk).first()
        if obj is None:
            return None, Response(
                {'detail': 'Item de plan no encontrado.'},
                status=status.HTTP_404_NOT_FOUND,
            )
        return obj, None

    def get_bloque_competencia_or_404(self, pk):
        obj = BloqueCompetencia.objects.select_related(
